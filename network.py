...
class ResNet_ssl(nn.Module):
    ...
    def forward(self, x, batch_size, is_training=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        x = self.fc(x)
        if is_training==True:
            sup_fm = fm[:batch_size]

            unsup_fm = fm[batch_size:]
            sup_logits = x[:batch_size]
            unsup_logits = x[batch_size:]
            return sup_fm, unsup_fm, sup_logits, unsup_logits
        else:

            logits = x
            return logits

def ResNet18_ssl(tsx_num_class):
    model = ResNet_ssl(BasicBlock, [2,2,2,2], num_classes=tsx_num_class)
    return model
