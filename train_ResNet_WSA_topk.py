...
if __name__ == '__main__':
    ...
    batch_size = {'labeled': 10,
                  ...
                  'un':50}
    cate_num = 10
    save_model_path = './MSTAR_128/' + datasetnum +'/' + range_ratio +'/model_top3/slc_joint_deeper_img_' + str(datasetnum) + '_FR_'
    ...
    f_acc = open(result_dir +'/all_top3_accuracy.txt','w')
    u_acc = open(result_dir + '/all_u_top3_accuracy.txt', 'w')
    ...
    for epoch in range(epoch_num):
        ...
        for (data_labeled, data_unlabeled) in train_loader:
            ...
            _,_,sup_logits, unsup_logits = net_joint(all_img, labels.shape[0], is_training=True)

            weak_logits = unsup_logits[:weak_img.shape[0]]
            strong_logits = unsup_logits[weak_img.shape[0]:]
            del unsup_logits
            pseudo_label = F.softmax(weak_logits.detach(), dim=-1)

            max_probs, targets_u = torch.max(pseudo_label, dim=-1)

            threshod = 0.8
            mask = max_probs.ge(threshod).float()
            unsup_ce_loss = (F.cross_entropy(strong_logits, targets_u, reduce=False) * mask).mean()
            unsup_topk_loss = (topk_criterion(strong_logits,targets_u)* mask).mean()
        ...
        data_num = 0
        val_loss = 0.0
        iter_val = iter(dataloaders['un'])
        for j in range(len(dataloaders['un'])):
        ...
        val_loss /= len(dataloaders['un'])
        val_acc = acc_num / data_num
        print(val_acc)
        u_acc.write(str(val_acc.item()) + '\n')
        acc_num = 0.0
        data_num = 0
        val_loss = 0.0
